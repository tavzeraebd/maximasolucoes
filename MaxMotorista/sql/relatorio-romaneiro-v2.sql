with por_entregas as (
	select entrega.codigo_motorista,
	       ...
	       sum(pedido.quant_total_caixas) as quant_total_caixas
	from fato_entregas entrega
	  inner join fato_carregamentos carregamento on carregamento.numero_carregamento = entrega.numero_carregamento and carregamento.codigo_motorista = entrega.codigo_motorista
	  inner join dim_nota_fiscal nota on nota.numero_carregamento = carregamento.numero_carregamento and nota.id_entrega = entrega.id_entrega
	  inner join fato_pedido_realizado pedido on pedido.numero_carregamento  = carregamento.numero_carregamento and pedido.numpederp = nota.numero_pedido and pedido.codigo_cliente = entrega.codigo_cliente --não tem todos os pedidos (a fato_pedido_realizado faz alguns filtros)
...
select por_entregas.id_entrega,
       round(sum(por_entregas.valor_atendido),2) as valor_entrega,
       round(sum(por_entregas.peso_total),2) as peso_entrega,
       round(sum(por_entregas.quant_total_caixas),2) as volume_total,       -- agora volume_total vem de quant_total_caixas
       round(sum(por_entregas.volume_total),2) as quant_total_caixas,       -- agora quant_total_caixas vem de volume_total
       ...
       case when por_entregas.raio_entrega > por_entregas.tolerancia_raio_entrega then 'S' ELSE 'N' end as entrega_fora_do_raio,
...
group by por_entregas.id_entrega,
       por_entregas.data_saida_carregamento,
       por_entregas.codigo_cliente,
       por_entregas.nome_cliente,
       por_entregas.cidade,
